...
if [ -n "$S3_BUCKET" ]; then
  ...
  /usr/bin/aws s3 sync s3://$S3_BUCKET /var/jenkins_home --exclude "*workspace/*" --exclude "*lastSuccessfulBuild/*" --exclude "*lastStableBuild/*" --exclude "*lastStableBuild/*" --exclude "*lastUnsuccessfulBuild/*"
